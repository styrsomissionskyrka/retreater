...
class AssetLoader
{
     ...
     * @var array
     */
    private static $assets = ["app"=>["js"=>"app.RT4NUJCH.js","nomodule"=>"app.OP3J2VKA.nomodule.js","deps"=>[],],];
     ...
     * @var string
     */
    private static $dev_client = 'var f=new URL(`ws://${window.WP_BUNDLER_HOST}:${window.WP_BUNDLER_PORT}`);a();function a(){let e=new WebSocket(f);e.addEventListener("close",n),e.addEventListener("open",s),e.addEventListener("message",i);function n(t){t.wasClean?r.info("Dev server closed and connection lost"):r.error(new Error(`Dev server disconnected: ${t.code} ${t.reason}`)),r.info("Retrying in five seconds"),setTimeout(a,5e3)}function s(){r.info("Dev server connection established")}function i(t){try{let o=w(t);switch(o.type){case"reload":u(o.files)?(r.info("Styles updated, refreshing."),window.requestIdleCallback(d)):(r.info("Source code updated, reloading."),window.requestIdleCallback(l),e.removeEventListener("close",n),e.removeEventListener("open",s),e.removeEventListener("message",i));break;default:r.info(`Unknown event sent with type ${o.type}`)}}catch(o){r.error(new Error("Could not parse incoming data from socket"))}}function l(){e.close(),window.location.reload()}function d(){let t=Array.from(document.querySelectorAll("link")).filter(o=>o.id.startsWith("wp-bundler."));for(let o of t){let c=`?reload=${new Date().getTime()}`;o.href=o.href.replace(/\?.*|$/,c)}}}function u(e){return e.every(n=>n.endsWith(".css"))}function w(e){if(typeof e.data!="string")throw new Error("Websocket data not passed as string");let n=JSON.parse(e.data);if(typeof n=="object"&&n!=null&&"type"in n)return n;throw new Error("Invalid message data")}var r={info(e){console.log(`[wp-bundler]: ${e}`)},error(e){console.error("[wp-bundler]: An error occured"),console.error(e)}};
';
}
